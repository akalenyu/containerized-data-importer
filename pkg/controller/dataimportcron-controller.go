...
var (
	// DataImportCronFailingGauge is the metric we use to alert about DataImportCrons failing
	DataImportCronFailingGauge = prometheus.NewGauge(
		prometheus.GaugeOpts{
			Name: "kubevirt_cdi_dataimportcron_failing_total",
...
func (r *DataImportCronReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if dataImportCron.DeletionTimestamp != nil {
		if err := r.cleanup(ctx, dataImportCron); err != nil {
			return reconcile.Result{}, err
		}
	...
	if res, err := r.update(ctx, dataImportCron); err != nil {
		return res, err
	}
	return r.checkFailingDataImportCrons()
}
...
func (r *DataImportCronReconciler) checkFailingDataImportCrons() (reconcile.Result, error) {
	numNotUpToDate := 0
	...
	DataImportCronFailingGauge.Set(float64(numNotUpToDate))
