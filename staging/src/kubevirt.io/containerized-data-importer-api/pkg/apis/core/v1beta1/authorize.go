...
func (dv *DataVolume) Authorize(requestNamespace, requestName string, sarClient SubjectAccessReviewsProxy, nsClient NamespaceProxy, dsClient DataSourceProxy, userInfo authentication.UserInfo) (bool, string, cloneSourceHandler, error) {
	...
	cloneSourceHandler, err := newCloneSourceHandler(dv, dsClient)
	if err != nil {
		if k8serrors.IsNotFound(err) && noTokenOkay {
			...
			return true, "", cloneSourceHandler, ErrNoTokenOkay
		}
		return false, "", cloneSourceHandler, err
	}
	...
	_, err = nsClient.Get(sourceNamespace)
	if err != nil {
		if k8serrors.IsNotFound(err) && noTokenOkay {
	...
	ok, reason, err := cloneSourceHandler.CloneAuthFunc(sarClient, sourceNamespace, sourceName, targetNamespace, userInfo)
	if err != nil {
		return false, reason, cloneSourceHandler, ErrNoTokenOkay
	}
	...
	return ok, reason, cloneSourceHandler, err
}
